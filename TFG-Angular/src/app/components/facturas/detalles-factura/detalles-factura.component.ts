...
export class DetallesFacturaComponent implements OnInit {
  ...
  public numero_albaranes: number;          //Variable para saber el numero de paginas de la factura.
  ...
  public id_factura: string;                //Variable para almacenar el id de la factura.
...
public FormularioPago = new FormGroup({
  ...
  //En esta función se hace una comprobación para saber si el factura que esta accediendo a este modulo
  //esta identificado en el sistena. Ademas se almacena el valor del uuid del factura.
  ngOnInit(): void {
  ...
  if(sessionStorage.getItem('emailLogin')!= null || sessionStorage.getItem('pass')!= null){
    ...
    this._route.params.subscribe(params => {
      ...
      //Se llama al metodo getFactura para obtener el factura.
      this.getFactura();

      //Se llama al metodo getDatos para obtener los datos de la empresa.
      this.getDatos();
  ...
  //Función para recuperar los datos de los facturas que se quiere mostrar en la factura, estos datos se le pasan al formulario.
  //Los datos del factura se recuperan utilizando el servicio de facturas, que se comunica con la base de datos
  //mediente el metodo getFactura.
  getFactura(){
  	this._facturaService.getFactura(this.id_factura).subscribe(
  		facturas => {
        for (let factura of facturas){

          this.fecha_inicial= factura.fecha_factura;

          //Almacena los datos recibidos de la base de datos en un objeto de tipo factura.
          this.factura = new Factura(factura.id,factura.id_cliente,factura.razon_social,factura.nombre_comercial,factura.fecha_factura,factura.pagado,factura.fecha_pagado,factura.generado); 
        
          this.getCliente(factura.id_cliente);
        }
  ...
  //Función para recuperar los datos de la empresa que se quieren mostrar en la factura, estos datos se le pasan al formulario.
...
//Función para mostrar un dialogo de confirmación para el borrado de un factura.
mostrarDialogo(factura: Factura): void {
  this.dialogo
    .open(DialogoConfirmacionComponent, {
      data: `Estas seguro de querer eliminar la factura  ` + factura.id + ` del cliente ` + factura.nombre_comercial+` ?`
    })
    .afterClosed()
...
//Función para eliminar un factura, el servicio se comunica con la API REST y borra el facturas de la base de datos.
private delete(factura: Factura) {
  this._facturaService.eliminarFactura(factura.id).subscribe(
    result=>{
      //this.refresh();
...
//Función para poner las facturas como pagadas.
mostrarPagado(){
  this.factura.pagado = "Si";
  this.factura.fecha_pagado = this.fecha.value;
  
  this._facturaService.editarFactura(this.factura).subscribe(
  result=>{
    //this.refresh();
    this._router.navigate(['/detalles-factura', this.factura.id]);
  }, error=>{
...
public generarPDF():void {
  ...
  this._pedidoService.getPedidoFacturado(this.id_factura).subscribe(
    pedidos => {

      this.numero_albaranes = pedidos.length;
      ...
      for (let pedido of pedidos){
        ...
        if(y >=231){
          y = 98; 
          this.mostrarPieDePaginaFactura(doc);
      ...
      this._lineaPedidoService.getLinea(pedido.id).subscribe(lineas=>{
      (lineas);
      ...
      //Bucle para recorrer todas las lineas del pedido.
        ...
        doc.text("Albaran número: ",26,y);
        doc.text(pedido.id+'',58,y);
        y = y + 6;
        doc.setFontSize(9);
        }
        ...
        if(x == this.numero_albaranes){
        this.mostrarPieDePaginaFactura(doc);

        //Metodo para generar el pdf.
        doc.save('factura-'+this.factura.id+'.pdf');
        }
    ...
    this.factura.generado = 'Si';
    
    this._facturaService.editarFactura(this.factura).subscribe(response=>{
    },
    error =>{
    }
    );
  ...
  let fecha_final : string = document.getElementById('fecha_oculta').innerHTML;
  ...
  doc.text('FACTURA:', 122,25);
  doc.text(this.factura.id+'',150,25);
  ...
  public mostrarPieDePaginaFactura(doc:jsPDF){
  ...
  let base_imponible = this.base_4iva + this.base_21iva + this.base_10iva;
  doc.text(base_imponible.toFixed(2)+' €',94,264);
  ...
  let importe_iva = this.importe_4iva + this.importe_21iva + this.importe_10iva;
  doc.text(importe_iva.toFixed(2)+' €',132,264);
  ...
  let precio_total = importe_iva + base_imponible;
  doc.text(precio_total.toFixed(2)+' €',160,252);
...
get fecha(){
return this.FormularioPago.get('fecha');
}
